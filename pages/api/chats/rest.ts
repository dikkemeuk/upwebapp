...
async function get(req: NextApiRequest, res: NextApiResponse) {
    ...
    console.log(id);
    //find messages with id less than id

    const messages = await prisma.cod2_cmdlog.findMany({where: {messageID: {lt: parseInt(id)}}, orderBy: { messageID: "desc" }, take: 500});
    const lowestID = messages[messages.length - 1].messageID;
